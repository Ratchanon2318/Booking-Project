...
$sql_rooms = "SELECT room_id, room_name FROM room";
$result_rooms = $objCon->query($sql_rooms);

// ดึงค่าจากฟอร์ม
$selected_room_id = isset($_GET['room_id']) ? $_GET['room_id'] : '';
$selected_booking_status = isset($_GET['booking_verify']) ? $_GET['booking_verify'] : '';
...
// สร้าง SQL Query ที่กรองข้อมูลตามค่าที่เลือกจากฟอร์ม
...
if ($selected_room_id != '') {
    $sql .= " AND b.room_id = '$selected_room_id'";
}

if ($selected_booking_status != '' && $selected_booking_status != 'All') {
    $sql .= " AND b.booking_verify = '$selected_booking_status'";
}
...
$sql_count = "SELECT COUNT(*) FROM booking WHERE 1=1";

if ($selected_room_id != '') {
    $sql_count .= " AND room_id = '$selected_room_id'";
}

if ($selected_booking_status != '' && $selected_booking_status != 'All') {
    $sql_count .= " AND booking_verify = '$selected_booking_status'";
}
    ...
    <div class="report-container">
        <div class="menu-container">
            <ul class="menu-links">
                ...
                <li><a href="cancelled_report.php">รายงานยกเลิกการจอง</a></li>
        ...
        <form method="get" action="report.php">
            ...
            <select name="room_id" id="room_id" required>
                ...
                <?php while ($room = $result_rooms->fetch_assoc()): ?>
                    <option value="<?php echo htmlspecialchars($room['room_id']); ?>" <?php echo ($selected_room_id == $room['room_id']) ? 'selected' : ''; ?>>
                        <?php echo htmlspecialchars($room['room_name']); ?>
                    </option>
                <?php endwhile; ?>
            </select>

            <select name="booking_verify" id="booking_verify" required>
                <option value="">-- เลือกสถานะ --</option>
                <option value="Pending" <?php echo ($selected_booking_status == 'Pending') ? 'selected' : ''; ?>>รออนุมัติ</option>
                <option value="Approved" <?php echo ($selected_booking_status == 'Approved') ? 'selected' : ''; ?>>อนุมัติ</option>
                <option value="All" <?php echo ($selected_booking_status == 'All') ? 'selected' : ''; ?>>ทั้งหมด</option>
            </select>
