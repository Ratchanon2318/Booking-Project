    ...
    if ($booking_id > 0) {
        ...
        if ($booking_data) {
            ...
            $sql_insert_cancelled = "INSERT INTO cancelled_booking 
                (booking_id, user_id, room_id, booking_date, desired_date, start_time, end_time, cancelled_by, cancelled_at) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
            ...
            if ($stmt_insert_cancelled->affected_rows > 0 && $stmt_delete_booking->affected_rows > 0) {
                // Success
                header("Location: report_user.php?message=cancel_success");
                exit();
            } else {
                // Error
                header("Location: report_user.php?message=cancel_error");
                exit();
            }
        }
